...
export class DataTests {
  ...
  static readonly cards = [
    ...
    new Card(2, '../../assets/card-2.jpeg', ''),
    new Card(3, '../../assets/card-3.jpeg', ''),
  ];
  static readonly categories: Category[] = [
    new Category('Acessórios', 1),
    new Category('Cards', 2),
    new Category('Colecionáveis', 3),
    new Category('Pacote de Cards', 4),
    new Category('Vestuário', 5)
  ];
  ...
  private static productRandom(title: string, desc: string, imgUrl: string): Product {
    return new Product(
      title, desc, randomFloat(0, 2500), imgUrl, randomFloat(0, 100),
      [DataTests.categories[randomInt(0, DataTests.categories.length - 1)]],
      randomBoolean(), ++DataTests.productId, randomInt(0, 10000), randomFloat(0, 5)
    );
  }
